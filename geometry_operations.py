...
# Import gcode_parser for are_points_close or define it locally if preferred
import gcode_parser # For are_points_close

def identify_candidate_regions_shapely(layer_wall_loops, cfg_min_area_thresh, cfg_coord_epsilon):
    # (Content of identify_candidate_regions_shapely function)
    # Ensure SHAPELY_AVAILABLE is checked in the main script before calling
    ...
    for i, loop_pts in enumerate(layer_wall_loops):
        ...
        try:
            ...
            if poly.geom_type == 'Polygon': current_polys_from_loop.append(poly)
            elif poly.geom_type == 'MultiPolygon':
                for p_part in poly.geoms: # Use .geoms for MultiPolygon
                    if p_part.geom_type == 'Polygon': current_polys_from_loop.append(p_part)
        ...
        for idx, p_valid in enumerate(current_polys_from_loop):
            if p_valid.area > cfg_min_area_thresh:
                ...
                shapely_polys_data.append({
                    'id': poly_id_str, 'shapely_poly': p_valid, 'area': p_valid.area,
                    'children_polys': [], 'parent_poly_id': None
                })
        processed_loop_indices.add(i)

    if not shapely_polys_data: return []
    ...
    for i in range(len(shapely_polys_data)):
        ...
        for j in range(i): # Only check polygons already processed (which are larger)
                ...
                if candidate_parent_poly.is_valid and current_poly.is_valid and \
                   candidate_parent_poly.contains(current_poly.representative_point()): # Efficient check first
                    ...
                    if abs(intersection_area - current_poly.area) < (current_poly.area * 0.01 + cfg_coord_epsilon): # Tolerance
                        if candidate_parent_poly.area < min_parent_area:
                            tightest_parent_data = potential_parent_candidate_data
                            min_parent_area = candidate_parent_poly.area
    ...
    for node_data in shapely_polys_data:
        ...
        direct_children_polys = node_data['children_polys']
        ...
        if not lamina_result.is_empty and lamina_result.area > cfg_min_area_thresh:
            if lamina_result.geom_type == 'Polygon':
                ...
                for p in lamina_result.geoms:
                    if p.area > cfg_min_area_thresh: all_laminae_shapely_polys.append(p)
        ...
        for shapely_poly in all_laminae_shapely_polys:
            if shapely_poly.is_valid and shapely_poly.exterior and shapely_poly.area > cfg_min_area_thresh:
                ...
                candidate_regions_for_classification.append({
                    'shapely_poly': shapely_poly, 'path_float': coords,
...
def generate_infill_segments_for_polygon(polygon, angle_degrees, line_spacing,
    ...
    # (Content of generate_infill_segments_for_polygon function)
    # Uses gcode_parser.are_points_close
    ...
    start_y_for_sweep = centroid_y - (num_lines / 2) * line_spacing
    ...
    rotated_infill_lines = [rotate(line, angle_degrees, origin=(centroid_x, centroid_y)) for line in infill_lines]
    ...
    def get_point_sweep_value(point):
        return (y_sort_multiplier * point[1], x_sort_multiplier * point[0])
...
def generate_and_mask_grid(classified_regions, wall_loops, travel_segments,
    ...
    # (Content of generate_and_mask_grid function)
    if not classified_regions:
        print("No classified regions to mask against.")
        return []

    all_coords = []
    for loop in wall_loops: all_coords.extend(loop)
    ...
    infill_polygons = [r['shapely_poly'] for r in classified_regions if r['contains_infill']]
    ...
    if cfg_infill_region_shrink_offset > cfg_coord_epsilon:
        try:
            ...
            if not shrunk_infill_mask.is_empty and shrunk_infill_mask.is_valid:
                ...
                print(f"Warning: Buffering infill mask by -{cfg_infill_region_shrink_offset}mm resulted in empty/invalid. Keeping original.")
        except Exception as e:
            print(f"Error buffering infill mask: {e}. Keeping original.")
    ...
    while current_x < max_x:
        ...
        while current_y < max_y:
            grid_cell = Polygon([
                (current_x, current_y), (current_x + cfg_grid_cell_size, current_y),
                (current_x + cfg_grid_cell_size, current_y + cfg_grid_cell_size),
                (current_x, current_y + cfg_grid_cell_size), (current_x, current_y)
            ])
            try:
                ...
                for poly_part in polys_to_add:
                    if poly_part.geom_type == 'Polygon' and poly_part.area > cfg_min_area_thresh / 100.0:
                        clipped_grid_cells_data.append({
                            ...
                            'infill_segments': [], 'infill_angle': None,
                            'cell_infill_start_pt': None, 'cell_infill_end_pt': None,
